...
// MAGIC ## Configuration
// MAGIC 
// MAGIC To configure a Key Vault-backed secret scope, use this notebook as a reference: [Configure Key Vault-backed Secret Scope]($./Configurations/Key-Vault-Backed-Secret-Scopes)
// MAGIC 
...
// MAGIC 
// MAGIC <img alt="Caution" title="Caution" style="vertical-align: text-bottom; position: relative; height:1.3em; top:0.0em" src="https://files.training.databricks.com/static/images/icon-warning.svg"/> While the secret scopes used throughout this notebook will not be available outside the instructor-led classroom, [this file](https://files.training.databricks.com/courses/adbcore/commonfiles/sales.csv) can be used if you wish to replicate this with your own remote filesystem, or you can use the file located at `/mnt/training/enb/commonfiles/sales.csv` after running the `Classroom-Setup` notebook.
...
// MAGIC %python
// MAGIC 
// MAGIC MOUNTPOINT = "/mnt/" + username + "/demo"
// MAGIC 
...
val MOUNTPOINT = "/mnt/" + username + "/demo"
...
try
  {
    sales2004DF.write.mode("overwrite").parquet(MOUNTPOINT + "/sales2004DF-" + username)
  } 
catch
  {
  println("Job aborted")
...
// MAGIC %md
// MAGIC ## Writing Directly to Files using SAS token
// MAGIC 
...
// MAGIC %python
// MAGIC 
// MAGIC CONTAINER = "sharedblob"
// MAGIC SASTOKEN = dbutils.secrets.get(scope="students", key="sharedblobwrite")
// MAGIC 
...
val SOURCE = "wasbs://sharedblob@" + dbutils.secrets.get(scope="students", key="sharedblobwrite") + ".blob.core.windows.net/"
...
// MAGIC %md
// MAGIC We can write to this blob as if it's mounted directly.

// COMMAND ----------

sales2004DF.write.mode("overwrite").parquet(SOURCE + "/sales2004DF-" + username)
