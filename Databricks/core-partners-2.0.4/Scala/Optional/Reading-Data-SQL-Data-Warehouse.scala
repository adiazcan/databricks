...
// MAGIC 
// MAGIC First, make sure that you have the JDBC Connection String connected to a running Azure Data Warehouse containing the Sample Data. You can create such a Data Warehouse by working through the notebook `Configurations/Azure-Data-Warehouse`.
// MAGIC 
// MAGIC Then, run the following cells to configure our "classroom."
...
val tableName = "dbo.DimProduct"
val jdbcURL = "jdbc:sqlserver://tt-sql-dw-2.database.windows.net:1433;database=AdWrks2;user=tt-sql-dw-2@tt-sql-dw-2;password={CORRECT-horse-battery-staple};encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.database.windows.net;loginTimeout=30;"
...
val exampleTwoDF = spark.read.jdbc(
  jdbcURL,                      // the JDBC URL
  tableName,                    // the name of the table
  "productkey",                         // the name of a column of an integral type that will be used for partitioning.
  1,                            // the minimum value of columnName used to decide partition stride.
  606,                       // the maximum value of columnName used to decide partition stride
