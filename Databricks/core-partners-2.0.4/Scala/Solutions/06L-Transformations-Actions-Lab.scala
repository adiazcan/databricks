...
val df = spark.read.table("geo_for_lookup_csv")
display(df)
...
val countsPath = userhome + "/country-counts"
countryCounts.write.format("parquet").mode("overwrite").save(countsPath)
...
val parquetPath = userhome + "/geo_for_lookup"

// COMMAND ----------

// ANSWER
val parquetDF = spark.read.format("parquet").load(parquetPath)
...
// MAGIC 
// MAGIC ```
// MAGIC val schemaDefinition = StructType(
// MAGIC   List(
...
// MAGIC 
// MAGIC `val df = spark.read.schema(schemaDefinition)......`
// MAGIC 
...
val schema = StructType(
  List(
    ...
    StructField("countrycode3", StringType, false),
    StructField("StateProvince", StringType, false),
    StructField("PostalCode", StringType, false)
  )
)

val parquetDF = spark.read.schema(schema).format("parquet").load(parquetPath)
parquetDF.printSchema()
