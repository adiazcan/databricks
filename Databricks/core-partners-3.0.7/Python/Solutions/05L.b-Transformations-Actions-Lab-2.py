...
celciusDF = weatherDF.withColumn("TAVG_C", 
  when(col("UNIT") == "F", convertFtoC(col("TAVG")))
  .otherwise(col("TAVG")))
...
# ANSWER
from pyspark.sql.functions import mean, stddev, desc, month

monthlyAggDF = (celciusDF.groupBy(["NAME", month("DATE").alias("month")]).agg(
...
# ANSWER
finalDF = monthlyAggDF.toDF(*[x.lower() for x in monthlyAggDF.columns])
...
monthlyWeatherPath = userhome + "/monthly-weather"
(finalDF
  .write
  .format("parquet")
  .mode("overwrite")
...
withStatesDF = celciusDF.withColumn("STATE", substring("NAME", -5, 2))
