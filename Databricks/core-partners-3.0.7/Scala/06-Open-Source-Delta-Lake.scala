...
// MAGIC * Describe why Delta format was created
// MAGIC * Compare the advantages of Delta Lake over traditional data lakes
// MAGIC * Describe how Delta Lake ensures data reliability 
// MAGIC * Create a table using Delta Lake
// MAGIC * Save files using Delta Lake
// MAGIC * Use Delta Lake to UPSERT data into existing Delta Lake tables
...
display(spark.sql("SELECT * FROM delta.`" + DataPath + "` LIMIT 5"))
...
val upsertDF = spark.read.format("json").load("/mnt/training/enb/commonfiles/upsert-data.json")
display(upsertDF)
...
// MAGIC * [Delta Transaction Log Deep Dive](https://databricks.com/blog/2019/08/21/diving-into-delta-lake-unpacking-the-transaction-log.html)
