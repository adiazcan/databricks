...
// MAGIC %md-sandbox
// MAGIC 
// MAGIC <h2><img src="https://files.training.databricks.com/images/105/logo_spark_tiny.png"> Micro-Batches as a Table</h2>
// MAGIC 
...
val schema = "Arrival_Time BIGINT, Creation_Time BIGINT, Device STRING, Index BIGINT, Model STRING, User STRING, geolocation STRUCT<city: STRING, country: STRING>, gt STRING, id BIGINT, x DOUBLE, y DOUBLE, z DOUBLE"
...
val dataPath = "/mnt/training/definitive-guide/data/activity-json/streaming"
...
// MAGIC %md
// MAGIC <h2><img src="https://files.training.databricks.com/images/105/logo_spark_tiny.png"> Differences between Static and Streaming Writes</h2>
// MAGIC 
// MAGIC The following cell demonstrates batch logic to append data from a static read.

// COMMAND ----------

val outputPath = userhome + "/static-write"
...
val outputPath = userhome + "/streaming-concepts"
...
// MAGIC 
// MAGIC When defining a streaming write, the `trigger` method specifies when the system should process the next set of data. The example above uses the default, which is the same as `.trigger(Trigger.ProcessingTime("0 seconds"))`.
// MAGIC 
...
// MAGIC %md-sandbox
// MAGIC 
// MAGIC <h2><img src="https://files.training.databricks.com/images/105/logo_spark_tiny.png"> Managing & Monitoring Streaming Queries</h2>
// MAGIC 
...
for (s <- spark.streams.active)          // Iterate over all streams
  println(s.id)                          // Print the stream's id and name
...
val myStream = "streaming_display"
