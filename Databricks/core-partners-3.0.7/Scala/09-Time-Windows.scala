...
val inputPath = "/mnt/training/definitive-guide/data/activity-json/streaming"
...
val streamingDF = spark
  ...
  .load(inputPath)
  .select(($"Creation_Time"/1E9).alias("time").cast("timestamp"),
        $"gt".alias("action"))
...
try {
  val sortedDF = streamingDF.orderBy($"time".desc)
  display(sortedDF)
} catch {
...
// MAGIC %md
// MAGIC 
// MAGIC <h2><img src="https://files.training.databricks.com/images/105/logo_spark_tiny.png"> What is Time?</h2>
// MAGIC 
