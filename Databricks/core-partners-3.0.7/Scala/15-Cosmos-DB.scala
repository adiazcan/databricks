...
// MAGIC 
// MAGIC Cosmos DB requires some dependencies; details on configuration can be found [here]($./Configurations/Cosmos-DB-Setup).
...
val PrimaryKey = dbutils.secrets.get(scope="instructor", key="cosmoskey")
val URI = dbutils.secrets.get(scope="instructor", key="cosmosuri")
