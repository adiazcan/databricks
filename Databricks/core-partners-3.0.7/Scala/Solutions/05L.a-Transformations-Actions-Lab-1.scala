...
// MAGIC -- ANSWER
// MAGIC 
// MAGIC CREATE OR REPLACE TEMP VIEW unique_names
// MAGIC AS (
...
// MAGIC %sql
// MAGIC -- ANSWER
// MAGIC SELECT * FROM unique_names
...
// MAGIC %sql
// MAGIC -- ANSWER
// MAGIC SELECT COUNT(*) FROM unique_names
...
// MAGIC -- ANSWER
// MAGIC SELECT (
// MAGIC   SELECT COUNT(DISTINCT NAME, STATION, LATITUDE, LONGITUDE, ELEVATION)
// MAGIC   FROM weather
// MAGIC   ) = (
// MAGIC   SELECT COUNT(*) FROM unique_names
// MAGIC   )
...
val stationAggDF = weatherDF.groupBy("NAME").agg(
...
// MAGIC -- ANSWER
// MAGIC 
// MAGIC CREATE OR REPLACE TEMP VIEW station_agg_temp
// MAGIC AS (
// MAGIC   SELECT NAME, MIN(DATE) date_min, MAX(DATE) date_max, COUNT(DATE) count 
// MAGIC   FROM weather
// MAGIC   GROUP BY NAME
// MAGIC   )
...
// MAGIC %sql
// MAGIC -- ANSWER
// MAGIC SELECT * FROM station_agg_temp
