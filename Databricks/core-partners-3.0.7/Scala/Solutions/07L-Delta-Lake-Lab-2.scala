...
// MAGIC %md
// MAGIC # Databricks Delta Time Machine & Optimization Lab
// MAGIC 
...
// ANSWER
val count = spark.sql("SELECT SUM(total_orders) FROM customer_counts WHERE Country='Sweden'").as[Long].collect()(0)
...
val preZorderQuery = spark.sql("SELECT * FROM customer_data_delta WHERE StockCode=22301 ").collect()
...
// ANSWER
val postZorderQuery = spark.sql("SELECT * FROM customer_data_delta WHERE StockCode=22301 ").collect()
...
// ANSWER
val preNumFiles = dbutils.fs.ls(DeltaPath + "/Country=Sweden").length
...
// ANSWER
val postNumFiles = dbutils.fs.ls(DeltaPath + "/Country=Sweden").length
