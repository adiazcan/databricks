...
# MAGIC %sql
# MAGIC select * from flight_delays_with_airport_codes_csv
...
# MAGIC %sql
# MAGIC select count(*) from flight_delays_with_airport_codes_csv
...
# MAGIC 
# MAGIC Let's execut another query that shows us how many rows do not have a value in the DepDel15 column.

# COMMAND ----------

# MAGIC %sql
# MAGIC select count(*) from flight_delays_with_airport_codes_csv where DepDel15 is null
...
dfFlightDelays = spark.sql("select * from flight_delays_with_airport_codes_csv")
...
# MAGIC dfflights <- sql("SELECT OriginAirportCode, OriginLatitude, OriginLongitude, Month, DayofMonth, cast(CRSDepTime as long) CRSDepTime, DayOfWeek, Carrier, DestAirportCode, DestLatitude, DestLongitude, cast(DepDel15 as int) DepDel15 from flight_delays_with_airport_codes_csv")
# MAGIC 
...
# MAGIC %sql
# MAGIC select * from flight_weather_with_airport_code_csv
...
# MAGIC %sql
# MAGIC select count(*) from flight_weather_with_airport_code_csv
...
# MAGIC %sql
# MAGIC select distinct WindSpeed from flight_weather_with_airport_code_csv
...
# MAGIC %sql
# MAGIC select distinct SeaLevelPressure from flight_weather_with_airport_code_csv
...
# MAGIC %sql
# MAGIC select distinct HourlyPrecip from flight_weather_with_airport_code_csv
...
dfWeather = spark.sql("select AirportCode, cast(Month as int) Month, cast(Day as int) Day, cast(Time as int) Time, WindSpeed, SeaLevelPressure, HourlyPrecip from flight_weather_with_airport_code_csv")
...
# MAGIC %md First, let's define a new function named `roundDown` that will round any number passed to it down.

# COMMAND ----------

# MAGIC %md Round Time down to the next hour, since that is the hour for which we want to use flight data. Then, add the rounded Time to a new column named "Hour", and append that column to the dfWeather DataFrame.
...
# MAGIC %md Persist the combined dataset to a new persistent global table.
