...
# MAGIC %md Be sure to save the pipeline to disk so we can retrieve it later.
...
bestModel.write().overwrite().save(model_name)
...
# MAGIC %md Next, remove the zip file of the model if it exists.

# COMMAND ----------

# Can remove an old model file, if needed.
dbutils.fs.rm( "/tmp/flight_delays.zip")
dbutils.fs.rm( "/dbfs/FileStore/flight_delays.zip")

# COMMAND ----------

# MAGIC %md Export a copy of the model to the tmp directory.

# COMMAND ----------

bestModel.write().overwrite().save("/tmp/models/pipelineModel")

# COMMAND ----------

# MAGIC %md Now zip up the directory and save the zip file to the Azure Databricks file store. This will allow us to download and save the exported model.

# COMMAND ----------

# MAGIC %sh
# MAGIC zip -r /tmp/flight_delays.zip /dbfs/tmp/models/pipelineModel*
# MAGIC cp /tmp/flight_delays.zip /dbfs/FileStore/flight_delays.zip

# COMMAND ----------

# MAGIC %md Finally, download the zip file by removing everything in the URL bar above after `https://[location].azuredatabricks.net/` with `files/flight_delays.zip`, then press ENTER. Don't worry, you won't be redirected to another page. It should download right away.
# MAGIC 
# MAGIC Example: `https://eastus.azuredatabricks.net/files/flight_delays.zip`
